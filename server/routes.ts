...
const migrationGuard = (e: unknown): void => {
  ...
  if (/relation .* does not exist|column .* does not exist|could not find .* column|schema cache/i.test(msg)) {
    throw new HError(500, 'MIGRATION_NEEDED', 'Your database needs the Book Engine + Editor update: run SUPABASE_MIGRATION_02.sql in your Supabase SQL editor, then try again.');
  }
};
...
const bookJson = (b: BookRow) => ({
  ...
  // Tolerate pre-migration rows (columns absent until SUPABASE_MIGRATION_02 runs).
